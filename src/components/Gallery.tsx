...
const Gallery : React.FC = () => {
    ...
    const allHamsterCollection : IHamster[] = [];
    const [open, setOpen] = React.useState(false);
    const [initialHamster, setInitialHamster] = React.useState<IHamster>(hamster)
    const [allHamsters, setHamsters]: [IHamster[], (posts: IHamster[]) => void] = React.useState(allHamsterCollection);
    const [loading, setLoading] : [boolean, (loading : boolean) => void] = React.useState<boolean>(true);

    useEffect(() => {
        async function FetchAllGalleryHamsters(){
            await axios.get<IHamster[]>(`/hamsters/`, {
                headers : {
    ...
    const deleteHamster = (id : string) => {
        // Here we call the API to delete the Hamster from the database/collection
        handleClose();
        axios.delete(`/hamsters/${id}`)
        .then(response => {
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        axios.post(`/hamsters`, initialHamster)
        .then(response => {
            //setHamsters({ })
        });
    }
